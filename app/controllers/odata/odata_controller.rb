module Odata
  class OdataController < ApplicationController
    ...
    #prepend_view_path 'app/views/odata'
    ...
    helper_method :odata_atom_feed, :odata_atom_entry, :odata_json_feed, :odata_json_entry
    ...
    %w{service metadata resource}.each do |method_name|
      define_method(:"redirect_to_#{method_name}") do
        # redirect_to(send(:"odata_#{method_name}_url"))
    ...
    def resource
      ...
      when Odata::Core::Segments::LinksSegment.segment_name
        ...
        respond_to do |format|
          format.xml  { render :inline => "xml.instruct!; @results.empty? ? xml.links('xmlns' => 'http://schemas.microsoft.com/ado/2007/08/dataservices') : xml.links('xmlns' => 'http://schemas.microsoft.com/ado/2007/08/dataservices') { @results.each { |r| xml.uri(odata_engine.resource_url(r[1])) } }", :type => :builder }
    ...
    def odata_atom_feed(xml, query, results, options = {})
      results_href, results_url = begin
        if base_href = options.delete(:href)
          [base_href.to_s, odata_engine.resource_url(base_href.to_s)]
        else
          [query.resource_path, odata_engine.resource_url(query.to_uri)]
        end
      end

      results_title = options.delete(:title) || results_href

      xml.tag!(:feed, { "xml:base" => odata_engine.service_url }.merge(options[:hide_xmlns] ? {} : @@odata_atom_xmlns)) do
        ...
        unless results.empty?
          ...
          results.each do |result|
            odata_atom_entry(xml, query, result, options.merge(:hide_xmlns => true, :href => results_href))
          end
        end
    ...
    def odata_atom_entry(xml, query, result, options = {})
      ...
      result_url = odata_engine.resource_url(result_href)
      ...
      xml.tag!(:entry, {}.merge(options[:hide_xmlns] ? {} : @@odata_atom_xmlns)) do
              ...
              xml.tag!(:link, navigation_property_attrs) do
                xml.m(:inline, :type => navigation_property_attrs[:type]) do
                  if navigation_property.to_end.multiple?
                    odata_atom_feed(xml, query, navigation_property.find_all(result), options.merge(:entity_type => navigation_property.to_end.entity_type, :expand => options[:expand][navigation_property]))
                  else
                    odata_atom_entry(xml, query, navigation_property.find_one(result), options.merge(:entity_type => navigation_property.to_end.entity_type, :expand => options[:expand][navigation_property]))
                  end
                end
              end
            else
    ...
    def odata_json_feed(query, results, options = {})
      results_json = results.collect { |result| odata_json_entry(query, result, options.merge(:d => false, :deferred => false)) }
    ...
    def odata_json_entry(query, result, options = {})
      entity_type = options[:entity_type] || query.data_services.find_entity_type(result.class)
      raise Odata::Core::Errors::EntityTypeNotFound.new(query, result.class.name) if entity_type.blank?

      resource_uri = odata_engine.resource_url(entity_type.href_for(result))
          ...
          entity_type.navigation_properties.sort_by(&:name).each do |navigation_property|
            unless %w{__deferred __metadata}.include?(navigation_property.name.to_s)
              ...
              _json[navigation_property.name.to_s] = begin
                if (options[:expand] || {}).keys.include?(navigation_property)
                  if navigation_property.to_end.multiple?
                    odata_json_feed(query, navigation_property.find_all(result), options.merge(:entity_type => navigation_property.to_end.entity_type, :expand => options[:expand][navigation_property], :d => false))
                  else
                    odata_json_entry(query, navigation_property.find_one(result), options.merge(:entity_type => navigation_property.to_end.entity_type, :expand => options[:expand][navigation_property], :d => false))
                  end
                else
                  {
                      "__deferred" => {
