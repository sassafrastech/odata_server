    ...
    class EntityType < Odata::AbstractSchema::EntityType
      ...
      def initialize(schema, active_record, options = {})
        ...
        Odata::AbstractSchema::Serializable.atom_element_names.each do |atom_element_name|
          ...
          odata_entity_type_property_name = :"atom_#{atom_element_name}_property"
          
          if @active_record.respond_to?(odata_active_record_method_name)
            result = @active_record.send(odata_active_record_method_name)
            next unless result.is_a?(Symbol)
            
            property = self.properties.find { |p| p.name == result.to_s }
            next if property.blank?
              
            self.send(:"#{odata_entity_type_property_name}=", property)
          elsif !@active_record.instance_methods.include?(odata_active_record_method_name.to_s) && @active_record.column_names.include?(atom_element_name.to_s)
